...
# - installe (ou remplace) la tâche de nuit : chaque jour à 3h17, git pull puis run_nuit.sh.
...
chmod 600 "$ENV"
LIGNE="17 3 * * * /bin/bash -lc 'cd \$HOME/webautonomos && git checkout -q main && git pull -q --ff-only && bash _tools/seo_pipeline/run_nuit.sh >> \$HOME/nuit.log 2>&1'"
